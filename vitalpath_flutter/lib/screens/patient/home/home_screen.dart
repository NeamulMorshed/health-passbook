...
class _MemberStatus {
  ...
  const _MemberStatus({required this.color, required this.tooltip});
}
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return userAsync.when(
      ...
      data: (user) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (context.mounted) maybeShowOnboardingTour(context);
        });
...
class _PendingInviteBanner extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final invites =
        ref.watch(pendingInvitesForEmailProvider(email)).asData?.value ?? [];
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          ...
          ref.invalidate(
              patientAppointmentsProvider((patientId: user.uid, limit: 50)));
            ...
            SliverPadding(
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                  // ── FRESHNESS ─────────────────────────────────────────────
                  FreshnessTimestamp(
                      lastUpdated: ref.watch(_homeLastRefreshedProvider)),
                  const SizedBox(height: 12),

                  // ── GET STARTED (new users only) ─────────────────────────
                  if ((medsAsync.asData?.value.isEmpty ?? false) &&
                      (apptsAsync.asData?.value.isEmpty ?? false)) ...[
                    _GetStartedGuide(),
                    const SizedBox(height: 12),
                  ],
...
class _CaregiversActiveBanner extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final connections =
        ref.watch(patientCaregiverConnectionsProvider(uid)).asData?.value ?? [];
    final connected =
        connections.where((c) => c.status == 'connected').toList();
    if (connected.isEmpty) return const SizedBox.shrink();

    final names = connected
        .map((c) => c.caregiverName?.split(' ').first ?? c.caregiverEmail)
        .take(2)
        .join(' & ');
...
class _TimeContextualCard extends StatelessWidget {
  ...
  const _TimeContextualCard(
      {required this.uid, required this.medsAsync, required this.mealsAsync});
    ...
    if (h >= 5 && h < 11) {
      ...
      return _ContextCard(
        ...
        body: dueMeds.isNotEmpty
            ? 'You have ${dueMeds.length} medicine${dueMeds.length == 1 ? '' : 's'} to take this morning.'
            : loggedTypes.contains(AppConstants.mealBreakfast)
                ? 'Breakfast done! Morning medicines all taken.'
                : 'Start your day — take your medicines and log breakfast.',
        onAction: () => dueMeds.isNotEmpty
            ? context.go('/care')
            : showLogMealSheet(context, uid,
                initialType: AppConstants.mealBreakfast),
      );
    } else if (h >= 11 && h < 14) {
      return _ContextCard(
        icon: HugeIcon(
        ...
        actionLabel: loggedTypes.contains(AppConstants.mealLunch)
            ? 'Log Vitals'
            : 'Log Lunch',
        ...
        onAction: () => loggedTypes.contains(AppConstants.mealLunch)
            ? context.go('/vitals')
            : showLogMealSheet(context, uid,
                initialType: AppConstants.mealLunch),
      );
    } else if (h >= 14 && h < 17) {
      return _ContextCard(
        icon: HugeIcon(
        ...
        body:
            'Good time for a walk or light activity. Logging steps keeps you on track.',
      ...
      return _ContextCard(
        ...
        body: eveningDue.isNotEmpty
            ? '${eveningDue.length} evening medicine${eveningDue.length == 1 ? '' : 's'} still due.'
            : loggedTypes.contains(AppConstants.mealDinner)
                ? 'Evening all done — great health day!'
                : 'Log dinner to complete your daily record.',
        onAction: () => eveningDue.isNotEmpty
            ? context.go('/care')
            : showLogMealSheet(context, uid,
                initialType: AppConstants.mealDinner),
      );
    } else {
    ...
    return BentoCard(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(
          ...
          Text(body,
              style: const TextStyle(
                  fontSize: 12, color: AppColors.foreground, height: 1.4)),
...
class _RefillCountdownCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final m in meds) {
      ...
      final dailyDoses =
          m.reminderTimes.isNotEmpty ? m.reminderTimes.length : 1;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text(urgent ? 'Refill urgently needed' : 'Medicine refill needed',
              style: TextStyle(
                  fontSize: 13, fontWeight: FontWeight.w600, color: cardColor)),
        ]),
        const SizedBox(height: 8),
        ...low.take(3).map((med) {
          final c = med.daysLeft <= 3
          ...
          return Padding(
            ...
            child: Row(children: [
              Expanded(
                  child: Text(med.name, style: const TextStyle(fontSize: 12))),
              Container(
      ...
      child: Row(children: [
        ...
        Expanded(
          ...
          Row(children: [
            ...
            // Phase 9 (a11y): 44dp tap target + Semantics label
            Semantics(
              button: true,
              label: 'About Health Points',
              child: InkWell(
                onTap: () => _showHPInfoSheet(context, gamProfile),
                  ...
                  child: Center(
                    child: Text(days[i],
                        style: TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.w600,
                            color: filled ? color : AppColors.mutedForeground)),
                  ),
                );
              })),
        ])),
...
void _showHPInfoSheet(BuildContext context, GamificationProfile? gamProfile) {
  final hp = gamProfile?.hp ?? 0;
  final level = gamProfile?.level ?? 1;
  final progress = gamProfile?.levelProgress ?? 0.0;
  final hpToNext = gamProfile?.hpToNextLevel ?? GamificationProfile.hpPerLevel;
  final streak = gamProfile?.medStreak ?? 0;
              ...
              const Expanded(
                child: Column(
                    ...
                    children: [
                      ...
                      Text('Your health engagement score',
                          style: TextStyle(
            ...
            BentoCard(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 6),
                    Text('$hpToNext HP to Level ${level + 1}',
                        style: const TextStyle(
                            fontSize: 11, color: AppColors.mutedForeground)),
                  ]),
            ),
            const SizedBox(height: 16),
            const Text('How you earn HP',
                style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
            const SizedBox(height: 10),
            _HPInfoRow(
                icon: HugeIcons.strokeRoundedMedicine01,
                label: 'Log a dose on time',
                value: '+10 HP'),
            _HPInfoRow(
                icon: HugeIcons.strokeRoundedFire,
                label: '$streak-day streak bonus',
                value: streak > 0 ? '+${streak * 2} HP/day' : 'Start a streak'),
            _HPInfoRow(
...
class _HPInfoRow extends StatelessWidget {
...
class _FamilyStatusBar extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final legendDismissed = ref.watch(_statusLegendDismissedProvider);

    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('Your Family',
          style: TextStyle(
              fontSize: 13,
              fontWeight: FontWeight.w600,
              color: AppColors.mutedForeground)),
      ...
      if (!legendDismissed) ...[
        _StatusLegendCard(
          onDismiss: () =>
              ref.read(_statusLegendDismissedProvider.notifier).state = true,
        ),
        const SizedBox(height: 8),
      ],
      SizedBox(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (_, i) =>
              _FamilyStatusChip(uid: uid, member: members[i]),
        ),
      ),
      const SizedBox(height: 12),
    ]);
...
class _FamilyStatusChip extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final status = _memberStatus(medsAsync);
  ...
  _MemberStatus _memberStatus(AsyncValue<List<Medicine>> medsAsync) {
    final meds = medsAsync.asData?.value;
    if (meds == null) {
      return const _MemberStatus(
          color: AppColors.mutedForeground, tooltip: 'Loading medicines');
    }
    final active = meds.where((m) => m.isActive).toList();
    if (active.isEmpty) {
      return const _MemberStatus(
          color: AppColors.mutedForeground, tooltip: 'No active medicines');
    }
    if (active.any((m) => m.hasMissedSlot && !m.hasDueSlot)) {
      return const _MemberStatus(
          color: AppColors.destructive, tooltip: 'Missed dose');
    }
    if (active.any((m) => m.hasDueSlot)) {
      return const _MemberStatus(
          color: AppColors.warning, tooltip: 'Dose due soon');
    }
    if (active.every((m) => m.takenToday)) {
      return const _MemberStatus(
          color: AppColors.success, tooltip: 'All doses taken');
    }
    return const _MemberStatus(
        color: AppColors.mutedForeground, tooltip: 'No reminders scheduled');
  }
}

// ── Get Started Guide (shown to new users with 0 medicines + 0 appointments) ──
class _GetStartedGuide extends StatelessWidget {
  const _GetStartedGuide();

  @override
  Widget build(BuildContext context) {
    return BentoCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Welcome to Omra!',
            style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
        const SizedBox(height: 4),
        const Text('Get set up in 3 quick steps.',
            style: TextStyle(fontSize: 13, color: AppColors.mutedForeground)),
        const SizedBox(height: 16),
        _GetStartedStep(
          icon: HugeIcons.strokeRoundedMedicine01,
          title: 'Add your first medicine',
          subtitle: 'Track doses and set reminders.',
        ...
        _GetStartedStep(
          ...
          subtitle: 'Book appointments and receive prescriptions.',
          onTap: () => context.go('/care'),
        ),
        const Divider(height: 1, color: AppColors.border),
        _GetStartedStep(
          ...
          subtitle: 'Let someone you trust keep an eye on your health.',
          onTap: () => context.go('/care'),
        ),
      ]),
    );
...
// ── Status Legend (dismissible, once per launch) ──────────────────────────────
class _StatusLegendCard extends StatelessWidget {
  final VoidCallback onDismiss;
  const _StatusLegendCard({required this.onDismiss});
        ...
        Expanded(
          child: Wrap(
            ...
            children: const [
              ...
              _LegendDot(color: AppColors.mutedForeground, label: 'No schedule'),
            ],
          ),
        ),
        const SizedBox(width: 4),
...
class _DailyAwarenessCard extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_DailyAwarenessCard> createState() =>
      _DailyAwarenessCardState();
}
...
class _DailyAwarenessCardState extends ConsumerState<_DailyAwarenessCard> {
  ...
  Widget build(BuildContext context) {
    ...
    if (hasMissedMed && hasMissedMeal) {
      message =
          'You missed your medicine and a meal — please ensure you take them on time to stay on track.';
    } else if (hasMissedMed) {
      message =
          'You missed one or more scheduled medicine doses. Please take them as soon as possible to stay on track.';
    } else {
      message =
          "You haven't logged a meal on time. Keeping track of your meals helps you stay on top of your health.";
    }
...
class _UpcomingTasksCardState extends ConsumerState<_UpcomingTasksCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final actionableMeds = meds
        .where((m) =>
            m.isActive &&
            (m.hasNoScheduledTimes
                ? !m.fullyTakenToday
                : m.hasDueSlot || m.hasMissedSlot))
        .toList();

    final currentMeal = _currentMealType();
    final loggedTypes = meals.map((m) => m.mealType).toSet();
    final hasActionableMeal =
        currentMeal != null && !loggedTypes.contains(currentMeal);
    final totalPending = actionableMeds.length + (hasActionableMeal ? 1 : 0);

    final header = Row(children: [
      const Expanded(
          child: Text('What needs to be done now',
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _MealStatusCard(
        ...
        onLogMeal: (type) =>
            showLogMealSheet(context, widget.uid, initialType: type),
      ),
      const SizedBox(height: 12),
    ]);
  }
...
class _MealStatusCard extends StatelessWidget {
  ...
  static Widget _mealIcon(String meal, Color color) {
    if (meal == AppConstants.mealBreakfast)
      return HugeIcon(
