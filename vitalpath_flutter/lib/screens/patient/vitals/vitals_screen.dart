    ...
    return Scaffold(
      ...
      body: vitalsAsync.when(
        ...
        data: (readings) {
          ...
          final latestBpDia = readings
              .where((r) => r.type == VitalType.bpDiastolic)
              .firstOrNull;
          final latestPulse =
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                _TrendsSection(patientId: patientId),
...
class _TrendsSection extends StatelessWidget {
  final String patientId;
  const _TrendsSection({required this.patientId});
...
class _LogVitalSheetState extends ConsumerState<_LogVitalSheet> {
  ...
  void _save() async {
    ...
    if (val == null || val <= 0) {
      showAppSnack(context, 'Please enter a valid positive number.',
          isError: true);
      return;
    }
    ...
    if (!VitalType.isNormal(_selectedType, val)) {
      ...
      final confirmed = await showDialog<bool>(
        ...
        builder: (_) => AlertDialog(
          title: const Text('Unusual Reading',
    ...
    try {
      ...
      if (mounted) {
        AppSnackBar.error(context, 'Failed to save reading. Try again.');
      }
    } finally {
...
class _VitalHistorySheet extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final allReadings =
        ref.watch(vitalsProvider(patientId)).asData?.value ?? [];
            ...
            Padding(
              ...
              child: Column(
                children: [
                  ...
                  VitalTrendChart(
                    patientId: patientId,
                    types: type == VitalType.bpSystolic
                        ? [VitalType.bpSystolic, VitalType.bpDiastolic]
                        : [type],
                    height: 120,
                    showAxis: false,
                  ),
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        if (isNormal) {
                          ...
                          statusIcon = Icon(Icons.error_rounded,
                              ...
                              onPressed: () async {
                                ...
                                if (confirmed == true && context.mounted) {
                                  try {
                                    ...
                                    if (context.mounted) {
                                      AppSnackBar.error(context, 'Failed to delete reading. Please try again.');
                                    }
                                  }
                                }
                              },
...
class _VitalStatusCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final missedMeds =
        activeMeds.where((m) => m.hasMissedSlot && !m.hasDueSlot).toList();
    final abnormalReadings =
        readings.where((r) => !VitalType.isNormal(r.type, r.value)).toList();
    ...
    final latestGlucose =
        readings.where((r) => r.type == VitalType.glucose).firstOrNull;
    final vitalSummary =
        _interpret(latestBpSys, latestBpDia, latestPulse, latestGlucose);
            ...
            Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                const Icon(Icons.local_fire_department_rounded,
                    size: 13, color: AppColors.caregiver),
                const SizedBox(width: 4),
                Text('$medStreak day streak',
                    style: const TextStyle(
                        fontSize: 12,
                        fontWeight: FontWeight.w600,
          ...
          Wrap(spacing: 8, runSpacing: 8, children: [
            if (latestBpSys != null && latestBpDia != null)
              _VitalChip(
                ...
                iconBuilder: (c) => HugeIcon(
                    ...
                    color: c,
                    size: 12),
                isNormal: VitalType.isNormal(
                        VitalType.bpSystolic, latestBpSys.value) &&
                    VitalType.isNormal(
                        VitalType.bpDiastolic, latestBpDia.value),
              ),
            if (latestPulse != null)
              _VitalChip(
                ...
                iconBuilder: (c) => HugeIcon(
                    ...
                    size: 12),
                isNormal:
                    VitalType.isNormal(VitalType.pulse, latestPulse.value),
              ),
            if (latestGlucose != null)
              _VitalChip(
                ...
                unit: 'mg/dL',
                iconBuilder: (c) => HugeIcon(
                    icon: HugeIcons.strokeRoundedDroplet, color: c, size: 12),
                isNormal:
                    VitalType.isNormal(VitalType.glucose, latestGlucose.value),
              ),
          ]),
        ],
      ]),
  ...
  String _interpret(VitalReading? bpSys, VitalReading? bpDia,
      VitalReading? pulse, VitalReading? glucose) {
    if (bpSys == null && pulse == null && glucose == null)
      return 'No vitals logged yet — use the button below to add your first reading.';
    final parts = <String>[];
    ...
    if (glucose != null &&
        !VitalType.isNormal(VitalType.glucose, glucose.value)) {
      parts.add(
          'glucose is ${glucose.value > 140 ? "above target" : "low"} (${glucose.value.toInt()} mg/dL)');
    }
    if (pulse != null && !VitalType.isNormal(VitalType.pulse, pulse.value)) {
      parts.add(
          'pulse is ${pulse.value > 100 ? "elevated" : "low"} (${pulse.value.toInt()} bpm)');
    }
